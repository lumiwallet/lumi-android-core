...
public class Sha256Hash implements Serializable, Comparable<Sha256Hash> {
    ...
    public static Sha256Hash of(byte[] contents, int start, int length) {
        return wrap(hash(contents, start, length));
    }
    ...
    public boolean equalsFromOffset(byte[] data, int offset, int size) {
        if ((data != null) && (offset >= 0) && (size >= 0) && (this.bytes.length > size)) {
