...
internal class Output(
    ...
    private fun validateDestinationAddress(destination: String) {
        ...
        require(
            destination.startsWith(BitcoinVaultParams.segwitAddressHrp!!) ||
                    Base58.decodeChecked(destination)[0] == BitcoinVaultParams.addressHeader.toByte() ||
                    Base58.decodeChecked(destination)[0] == BitcoinVaultParams.p2SHHeader.toByte()
    ...
    private fun decodeAndValidateAddress(address: String, addressType: ScriptType): ByteArray =
        when (addressType) {
            ScriptType.P2WPKH -> {
                SegwitAddress.fromBech32(BitcoinVaultParams, address).witnessProgram
            }
