                      ...
                      unsigned int key_size)
{
    ...
    unsigned char key_temp[SHA512_HASH_SIZE];
    int i;
    ...
    if (key_size == SHA512_BLOCK_SIZE) {
        ...
        if (key_size > SHA512_BLOCK_SIZE){
            num = SHA512_HASH_SIZE;
            Sha512Calculate(key, key_size, key_temp);
            key_used = key_temp;
    ...
    Sha512Initialise(&ctx->ctx_inside);
    Sha512Update(&ctx->ctx_inside, ctx->block_ipad, SHA512_BLOCK_SIZE);

    Sha512Initialise(&ctx->ctx_outside);
    Sha512Update(&ctx->ctx_outside, ctx->block_opad,
                  SHA512_BLOCK_SIZE);
...
{
    Sha512Update(&ctx->ctx_inside, message, message_len);
}

void hmac_sha512_final(hmac_sha512_ctx *ctx, unsigned char *mac,
                       unsigned int mac_size)
{
    unsigned char digest_inside[SHA512_HASH_SIZE];
    unsigned char mac_temp[SHA512_HASH_SIZE];

    Sha512Finalise(&ctx->ctx_inside, digest_inside);
    Sha512Update(&ctx->ctx_outside, digest_inside, SHA512_HASH_SIZE);
    Sha512Finalise(&ctx->ctx_outside, mac_temp);
