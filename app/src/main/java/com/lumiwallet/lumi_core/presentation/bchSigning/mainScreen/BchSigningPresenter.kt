package com.lumiwallet.lumi_core.presentation.bchSigning.mainScreen
...
@InjectViewState
class BchSigningPresenter : BasePresenter<BchSigningView>() {
    ...
    override fun onDestroy() {
        App.destroyBchSigningComponent()
        super.onDestroy()
    }
    ...
    fun addClick(headerType: Int) {
        when (headerType) {
            HeaderViewModel.HEADER_TYPE_INPUT -> {
                viewState.navigateToAddBchInputFragment()
            }
            HeaderViewModel.HEADER_TYPE_OUTPUT -> {
                viewState.navigateToAddBchOutputFragment()
            }
        }
    }
    ...
    fun onInputClick(input: InputViewModel) {
        viewState.navigateToEditBchInputFragment(input)

    }

    fun onOutputClick(output: Output) {
        viewState.navigateToEditBchOutputFragment(output)
    }
}