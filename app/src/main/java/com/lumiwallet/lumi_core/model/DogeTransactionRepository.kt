...
import com.lumiwallet.android.core.doge.transaction.TransactionBuilder
import com.lumiwallet.android.core.doge.transaction.UnspentOutput
...
class DogeTransactionRepository: DogeTransactionRepository{
        ...
        for (input in inputs) {
            transactionBuilder.from(
                UnspentOutput(
                    ...
                    input.script,
                    input.value,
                    PrivateKey.ofWif(input.keyAsWif, DogeNetParams)
                )
            )
        }
